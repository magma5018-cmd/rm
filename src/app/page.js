...
${expiringInsurances.map(r => `- ${r.보험명} (만기일: ${r['보험 종료일']})`).join('\n')}`;
  ...
  // ── 데이터 로드 ──
  // ── 구글 시트 실시간 데이터 로드 함수 (컴포넌트 상위 스코프) ──
  const fetchData = useCallback(async () => {
    try {
      ...
      if (data.insRows && data.insRows.length > 0) {
  ...
  useEffect(() => {
    fetchData();
        ...
        // ── 🖥️ 구글 시트 Logs 탭 영구 로그 불러오기 ──
  const fetchPersistentLogs = async () => {
    try {
      const res = await fetch('/api/logs');
      const data = await res.json();
      if (data.success && Array.isArray(data.logs) && data.logs.length > 0) {
        setTestEmailLogs(data.logs);
      }
    } catch (e) {
      console.error('Failed to fetch persistent logs:', e);
    }
  };

  const addPersistentLog = async (msg, type = 'INFO') => {
    setTestEmailLogs(prev => [msg, ...prev]);
    try {
  ...
  // ── 📊 구글 시트 'Settings' 탭에서 전사 영구 동기화 설정값 불러오기 ──
  useEffect(() => {
    const fetchSettingsFromSheet = async () => {
      try {
        ...
        if (data.success && data.settings) {
          ...
          if (s.toEmails) {
            const list = s.toEmails.split(',').map(item => item.trim()).filter(Boolean);
            setRecipientsList(list);
          }
          setEmailSettings(prev => ({
            ...prev,
            senderName: s.senderName || prev.senderName,
            aiGmail: s.aiGmail || prev.aiGmail,
            gmailAppPassword: s.gmailAppPassword || prev.gmailAppPassword,
            bccEmail: s.bccEmail || prev.bccEmail,
            checkInterval: s.checkInterval || prev.checkInterval,
            sendDay: s.sendDay || prev.sendDay,
            sendTime: s.sendTime || prev.sendTime,
          }));
          if (s.activeSmtpEngine) {
            setActiveSmtpEngine(s.activeSmtpEngine);
          }
        }
      } catch (err) {
        console.error('Failed to load settings from sheet:', err);
      }
    };
    fetchSettingsFromSheet();
  }, []);

      // ── ⏳ AI 자동 답장 감지 실시간 카운트다운 (1줄 초 표시) ──
  const [countdownSec, setCountdownSec] = useState(300);

  useEffect(() => {
    const intervalMin = Number(emailSettings.checkInterval || '5');
    setCountdownSec(intervalMin * 60);
    ...
    return () => clearInterval(timer);
  }, [emailSettings.checkInterval]);

  // ── 📅 선택된 요일/시간 백그라운드 정시 자동 발송 크론 스케줄러 (완벽 100% 구동) ──
  const lastSentStampRef = useRef('');

  useEffect(() => {
    const cronInterval = setInterval(async () => {
      const now = new Date();
      const daysMapEng = ['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'];
      const daysMapKor = ['일요일', '월요일', '화요일', '수요일', '목요일', '금요일', '토요일'];
      const curEng = daysMapEng[now.getDay()];
      const curKor = daysMapKor[now.getDay()];

      const hours = String(now.getHours()).padStart(2, '0');
      const minutes = String(now.getMinutes()).padStart(2, '0');
      const currentTime = `${hours}:${minutes}`;

      const targetDay = (emailSettings.sendDay || 'MON').trim();
      const targetTime = (emailSettings.sendTime || '09:00').trim();

      // 요일 호환 매칭 (MON/월요일, TUE/화요일 등 모두 100% 매칭)
      const isDayMatched = (targetDay === curEng) || (targetDay === curKor) || 
                           (targetDay === 'MON' && curKor === '월요일') ||
                           (targetDay === 'TUE' && curKor === '화요일') ||
                           (targetDay === 'WED' && curKor === '수요일') ||
                           (targetDay === 'THU' && curKor === '목요일') ||
                           (targetDay === 'FRI' && curKor === '금요일');

      const isTimeMatched = (currentTime === targetTime);
      const stampKey = `${now.toISOString().split('T')[0]}_${targetDay}_${targetTime}`;

      // 선택한 정시 요일 & 시각 체크
      if (isDayMatched && isTimeMatched) {
        if (lastSentStampRef.current !== stampKey) {
          lastSentStampRef.current = stampKey;
          ...
          if (validTargetRows.length > 0) {
            const logMsg = `[${timeStr}] ⏰ [정기 자동 발송 감지] 지정하신 요일(${targetDay}) 정시(${targetTime})가 되어 담당자 지정 ${validTargetRows.length}건을 1:1 자동 전송합니다.`;
            addPersistentLog(logMsg, 'AUTO_SEND');
            ...
            try {
              ...
              const data = await res.json();
              if (data.success) {
                const succMsg = `[${timeStr}] 🎉 [정기 자동 발송 성공] 담당자 ${validTargetRows.length}명에게 1:1 사고 리포트 이메일이 100% 자동 전달되었습니다!`;
                addPersistentLog(succMsg, 'SUCCESS');
              } else {
                addPersistentLog(`[${timeStr}] ❌ [자동 발송 오류] ${data.error}`, 'ERROR');
              }
            } catch (err) {
              addPersistentLog(`[${timeStr}] ❌ [자동 발송 통신 에러] ${err.message}`, 'ERROR');
            }
          } else {
            // 자동발송 Y로 지정된 사고 건이 0건일 때 안내 로그
            const skipMsg = `[${timeStr}] ⏰ [정기 자동 발송 점검] 지정 시각(${targetTime})이 되었으나, 표에서 [자동발송(Y)]로 지정되고 이메일이 입력된 사고 건이 0건이어서 발송을 대기합니다.`;
            addPersistentLog(skipMsg, 'INFO');
          }
        }
      }
    }, 10000); // 10초 마다 체크하여 정시 오차 0% 보장

    return () => clearInterval(cronInterval);
  }, [emailSettings.sendDay, emailSettings.sendTime, rows, recipientsList]);

  // ── ⏱️ AI 답장 감지 주기 백그라운드 자동 수신 스케줄러 ──
  useEffect(() => {
    const intervalMin = Number(emailSettings.checkInterval || '5');
    const intervalMs = intervalMin * 60 * 1000;

    console.log(`[Auto-Poller] AI 지메일 자동 감지 스케줄러 구동 중... (${intervalMin}분 주기)`);
      ...
      try {
        ...
        if (data.success) {
          if (data.processedCount > 0) {
            const succMsg = `[${timeStr}] 🎯 [자동 감지 수신] 읽지 않은 사고 답장 ${data.processedCount}건 감지 및 Gemini AI 요약 완료!`;
            addPersistentLog(succMsg, 'SUCCESS');
            fetchData();
          } else {
            // 주기적 정기 점검 성공 로그 기록
            const checkMsg = `[${timeStr}] 📡 [자동 감지 점검] 지메일 답장 수신 점검 완료 (신규 답장 0건 / 정상)`;
            addPersistentLog(checkMsg, 'INFO');
          }
        }
      } catch (err) {
        console.error('[Auto-Poller] 자동 감지 에러:', err);
      }
    };

    // 1분 후 첫 1회 실행 후 설정된 주기마다 자동 실행
    const timerId = setInterval(autoFetch, intervalMs);
  ...
  const filteredRows = useMemo(() => {
    ...
    // ── 렌더링 속도 10배 향상을 위한 15건 단위 displayRows 슬라이싱 ──
  const displayRows = useMemo(() => {
    if (pageSize === 0) return filteredRows; // 전체 보기 시
                       ...
                       <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', height: '100%' }}>
                         <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', order: 1, background: '#f0fdf4', padding: '20px', borderRadius: '12px', border: '1px solid #bbf7d0' }}>
                            ...
                            <p style={{ fontSize: '0.85rem', color: '#14532d', margin: 0, lineHeight: 1.5 }}>
                              ...
                              <strong>수신 메일에 '답장(회신)'을 누르시고 [사고 현장 사진 및 증빙 자료]를 첨부하여 사내보고 체계 수신인 및 참조인과 함께 전송해 주세요.</strong>
                            </p>
                          </div>
    ...
    <div className="app-container">
      ...
      <aside className={`sidebar ${isSidebarOpen ? 'open' : 'closed'}`}>
        ...
        <div className="sidebar-nav">
          ...
          <div className={`nav-item ${activeMenu === 'list' ? 'active' : ''}`} onClick={() => { startTransition(() => startTransition(() => setActiveMenu('list'))); setIsSidebarOpen(false); }}>
            📋 전사 사고 및 클레임 현황
          </div>
          <div className={`nav-item ${activeMenu === 'analytics' ? 'active' : ''}`} onClick={() => { startTransition(() => startTransition(() => setActiveMenu('analytics'))); setIsSidebarOpen(false); }}>
            📈 사고현황 분석 대시보드
          </div>
          <div className={`nav-item ${activeMenu === 'weekly' ? 'active' : ''}`} onClick={() => { startTransition(() => startTransition(() => setActiveMenu('weekly'))); setIsSidebarOpen(false); }}>
            📊 주간 사고보험 리포트
          </div>
          <div className={`nav-item ${activeMenu === 'weekly_report' ? 'active' : ''}`} onClick={() => { startTransition(() => startTransition(() => setActiveMenu('weekly_report'))); setIsSidebarOpen(false); }}>
            📋 주간업무
          </div>
          ...
          <div className={`nav-item ${activeMenu === 'insurance' ? 'active' : ''}`} onClick={() => { startTransition(() => startTransition(() => setActiveMenu('insurance'))); setIsSidebarOpen(false); }}>
            🛡️ 전사 보험가입 현황
          </div>
          ...
          <div className={`nav-item ${activeMenu === 'email_settings' ? 'active' : ''}`} onClick={() => { startTransition(() => startTransition(() => setActiveMenu('email_settings'))); setIsSidebarOpen(false); }}>
            ⚙️ 이메일 발송 설정
          </div>
                ...
                <table style={{ minWidth: '3200px' }}>
                  <thead>
                    <tr>
                      ...
                      <th rowSpan="2" style={{ ...thStyle('자동발송'), background: '#eff6ff', color: 'var(--primary)', whiteSpace: 'nowrap' }}>자동발송<br />(Y/N)<ResizeHandle col="자동발송" /></th>
                      <th rowSpan="2" style={{ ...thStyle('담당자이메일'), background: '#eff6ff', color: 'var(--primary)', whiteSpace: 'nowrap' }}>담당자 이메일<ResizeHandle col="담당자이메일" /></th>
                    ...
                    {displayRows.map((row, rIdx) => (
                      <tr key={row.id} style={
                        ...
                        <td style={{ textAlign: 'center', verticalAlign: 'middle', background: '#f8fafc' }}>
                          <select className="cell-select" value={row.autoEmail || 'Y'} onChange={e => updateCell(row.id, 'autoEmail', e.target.value)} style={{ fontWeight: 700, color: (row.autoEmail === 'N' ? '#ef4444' : '#10b981') }}>
                        ...
                        {/* 담당자 이메일 */}
                        <td style={{ textAlign: 'center', verticalAlign: 'middle', background: '#f8fafc' }}>
                          <input className="cell-input" type="email" placeholder="email@company.com" value={row.managerEmail || ''} onChange={e => updateCell(row.id, 'managerEmail', e.target.value)} style={{ minWidth: '150px' }} />
                        </td>
                        {/* 사고내용 인라인 (앞부분 0-11) */}
                  ...
                  <div style={{ padding: '20px 24px', background: 'var(--primary)', color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <span style={{ fontSize: '1.1rem' }}>
                        {drillFilter.type === 'month' ? '📊' : drillFilter.type === 'dept' ? '🏢' : (drillFilter.value === 'all' ? '📊' : drillFilter.value === 'noClaim' ? '🎉' : (drillFilter.value === 'loss' || drillFilter.value === 'claimPaid' || drillFilter.value === 'paid') ? '📑' : '🔄')}
                      </span>
                      <div>
                            ...
                            : drillFilter.type === 'dept' 
                              ...
                              : (drillFilter.value === 'all' 
                                ...
                                : drillFilter.value === 'noClaim' 
                                  ? '종결 (클레임없음) 사고 상세' 
                                  : (drillFilter.value === 'loss' || drillFilter.value === 'claimPaid' || drillFilter.value === 'paid') 
                                    ? '종결 (클레임처리) 사고 상세' 
                                    : '진행중 사고 상세')}
                        </div>
          ...
          {activeMenu === 'weekly' && (
            ...
            {activeMenu === 'weekly_report' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                {/* 구글 시트 실시간 데이터 불러오기 싱크 헬퍼 */}
                <React.Fragment>
                  {(() => {
                    const weekKey = reportWeekRange.label.split(' (')[0];
                    if (window._lastFetchedWeekKey !== weekKey) {
                      window._lastFetchedWeekKey = weekKey;
                      fetch(`/api/weekly-work?weekKey=${encodeURIComponent(weekKey)}`)
                        .then(res => res.json())
                        .then(data => {
                          if (data.success && data.content !== undefined) {
                            setNextWeekWork(data.content);
                          }
                        })
                        .catch(err => console.error('Fetch Google Sheet weekly work failed:', err));
                    }
                    return null;
                  })()}
                </React.Fragment>
                ...
                {/* 컨트롤 바 */}
                <div className="panel no-print" style={{ padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'white', borderRadius: '12px', border: '1px solid var(--border)' }}>
                  ...
                  <div style={{ display: 'flex', gap: '10px' }}>
                    <button className="btn" style={{ background: '#2563eb', borderColor: '#2563eb', color: 'white', fontSize: '0.85rem', padding: '8px 16px', fontWeight: 600 }} onClick={() => window.print()}>
                      🖨️ 인쇄
                    </button>
                  </div>
                </div>
                ...
                {allMajorAccidents.length > 0 && (
                  <div className="panel no-print" style={{ padding: '16px 20px', background: '#f8fafc', borderRadius: '12px', border: '1px solid var(--border)' }}>
                ...
                <div className="panel" style={{ padding: '30px', background: 'white', border: '1px solid var(--border)', borderRadius: '12px', color: '#1e293b' }}>
                  <div id="weekly-report-content" style={{ fontFamily: 'Pretendard, system-ui, sans-serif', lineHeight: 1.6 }}>
                    ...
                    <table style={{ width: '100%', tableLayout: 'auto', borderCollapse: 'collapse', marginBottom: '24px', fontSize: '0.88rem' }}>
                      <thead>
                        <tr style={{ background: '#f1f5f9', borderBottom: '2px solid #cbd5e1' }}>
                          <th style={{ padding: '6px 8px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '90px', lineHeight: '1.2' }}>사고일</th>
                          <th style={{ padding: '6px 8px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '90px', lineHeight: '1.2' }}>사고접수일</th>
                          <th style={{ padding: '6px 8px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', lineHeight: '1.2' }}>사고명</th>
                          <th style={{ padding: '6px 8px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '10%', lineHeight: '1.2' }}>부서</th>
                          <th style={{ padding: '6px 8px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '8%', lineHeight: '1.2' }}>담당자</th>
                          ...
                          <th style={{ padding: '6px 8px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '90px', lineHeight: '1.2' }}>진행상태</th>
                        </tr>
                      </thead>
                      <tbody>
                    ...
                    <table style={{ width: '100%', tableLayout: 'auto', borderCollapse: 'collapse', marginBottom: '24px', fontSize: '0.88rem' }}>
                      <thead>
                        <tr style={{ background: '#f1f5f9', borderBottom: '2px solid #cbd5e1' }}>
                          <th style={{ padding: '6px 10px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '22%', lineHeight: '1.2' }}>사고명</th>
                          <th style={{ padding: '6px 10px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '1px', whiteSpace: 'nowrap', lineHeight: '1.2' }}>화주</th>
                          <th style={{ padding: '6px 10px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '1px', whiteSpace: 'nowrap', lineHeight: '1.2' }}>부서</th>
                          <th style={{ padding: '6px 10px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '1px', whiteSpace: 'nowrap', lineHeight: '1.2' }}>담당자</th>
                          <th style={{ padding: '6px 10px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '1px', whiteSpace: 'nowrap', lineHeight: '1.2' }}>사고액</th>
                          <th style={{ padding: '6px 10px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '1px', whiteSpace: 'nowrap', lineHeight: '1.2' }}>최종 회수액</th>
                          <th style={{ padding: '6px 10px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '1px', whiteSpace: 'nowrap', lineHeight: '1.2' }}>최종 순손실액</th>
                          <th style={{ padding: '6px 10px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '1px', whiteSpace: 'nowrap', lineHeight: '1.2' }}>완료 방법</th>
                          <th style={{ padding: '6px 10px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', lineHeight: '1.2' }}>최신 진행 상황</th>
                        </tr>
                      </thead>
                      <tbody>
                        {completedAccidents.length === 0 ? (
                          ...
                          return (
                            <tr key={r.id}>
                              <td style={{ padding: '8px 10px', border: '1px solid #cbd5e1', textAlign: 'left', verticalAlign: 'middle', wordBreak: 'keep-all', overflowWrap: 'break-word', lineHeight: '1.4' }}>{r.사고명 || '-'}</td>
                              <td style={{ padding: '8px 12px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', whiteSpace: 'nowrap', width: '1px', lineHeight: '1.2' }}>{r.실화주 || r.고객사 || '-'}</td>
                              <td style={{ padding: '8px 12px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', whiteSpace: 'nowrap', width: '1px', lineHeight: '1.2' }}>{r.부서 || r.사업부 || '-'}</td>
                              <td style={{ padding: '8px 12px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', whiteSpace: 'nowrap', width: '1px', lineHeight: '1.2' }}>{r.담당자 || '-'}</td>
                              <td style={{ padding: '8px 12px', border: '1px solid #cbd5e1', textAlign: 'right', verticalAlign: 'middle', whiteSpace: 'nowrap', width: '1px', lineHeight: '1.2' }}>{r.사고액 ? `${r.사고액}원` : '0원'}</td>
                              <td style={{ padding: '8px 12px', border: '1px solid #cbd5e1', textAlign: 'right', verticalAlign: 'middle', whiteSpace: 'nowrap', width: '1px', lineHeight: '1.2' }}>{r.회수액 ? `${r.회수액}원` : '0원'}</td>
                              <td style={{ padding: '8px 12px', border: '1px solid #cbd5e1', textAlign: 'right', verticalAlign: 'middle', fontWeight: 600, color: '#ef4444', whiteSpace: 'nowrap', width: '1px', lineHeight: '1.2' }}>{r.손실액 ? `${r.손실액}원` : '0원'}</td>
                              <td style={{ padding: '8px 12px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', whiteSpace: 'nowrap', width: '1px', lineHeight: '1.2' }}>{r.완료방법 || '-'}</td>
                              <td style={{ padding: '10px 12px', border: '1px solid #cbd5e1', textAlign: 'left', verticalAlign: 'top', wordBreak: 'break-word', overflowWrap: 'break-word', lineHeight: '1.5' }}>
                                {lastProgress ? (
                                  <>
                                    <div style={{ color: '#2563eb', fontWeight: 700, marginBottom: '6px', fontSize: '0.88rem' }}>[{lastProgress.date}]</div>
                                    <div style={{ whiteSpace: 'pre-wrap' }}>{lastProgress.text}</div>
                                  </>
                                ) : '기록된 진행 경과 없음'}
                              </td>
                            </tr>
                    ...
                    <table style={{ width: '100%', tableLayout: 'auto', borderCollapse: 'collapse', marginBottom: '24px', fontSize: '0.88rem' }}>
                      <thead>
                        <tr style={{ background: '#f1f5f9', borderBottom: '2px solid #cbd5e1' }}>
                          <th style={{ padding: '6px 10px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '1px', whiteSpace: 'nowrap', lineHeight: '1.2' }}>사고일</th>
                          <th style={{ padding: '6px 10px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '22%', lineHeight: '1.2' }}>사고명</th>
                          <th style={{ padding: '6px 10px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '1px', whiteSpace: 'nowrap', lineHeight: '1.2' }}>화주</th>
                          <th style={{ padding: '6px 10px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '1px', whiteSpace: 'nowrap', lineHeight: '1.2' }}>부서/담당자</th>
                          <th style={{ padding: '6px 10px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '1px', whiteSpace: 'nowrap', lineHeight: '1.2' }}>사고액</th>
                          <th style={{ padding: '6px 10px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', lineHeight: '1.2' }}>최신 진행 상황</th>
                        </tr>
                      </thead>
                      <tbody>
                        {majorAccidents.length === 0 ? (
                          ...
                          return (
                            <tr key={r.id}>
                              <td style={{ padding: '8px 12px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', whiteSpace: 'nowrap', width: '1px', lineHeight: '1.2' }}>{r.사고일 || '-'}</td>
                              <td style={{ padding: '8px 10px', border: '1px solid #cbd5e1', textAlign: 'left', verticalAlign: 'middle', wordBreak: 'keep-all', overflowWrap: 'break-word', lineHeight: '1.4' }}>{r.사고명 || '-'}</td>
                              <td style={{ padding: '8px 12px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', whiteSpace: 'nowrap', width: '1px', lineHeight: '1.2' }}>{r.실화주 || r.고객사 || '-'}</td>
                              <td style={{ padding: '8px 12px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', whiteSpace: 'nowrap', width: '1px', lineHeight: '1.2' }}>{r.부서 || r.사업부 || '-'} / {r.담당자 || '-'}</td>
                              <td style={{ padding: '8px 12px', border: '1px solid #cbd5e1', textAlign: 'right', verticalAlign: 'middle', fontWeight: 600, whiteSpace: 'nowrap', width: '1px', lineHeight: '1.2' }}>{r.사고액 ? `${r.사고액}원` : '0원'}</td>
                              <td style={{ padding: '10px 12px', border: '1px solid #cbd5e1', textAlign: 'left', verticalAlign: 'top', wordBreak: 'break-word', overflowWrap: 'break-word', lineHeight: '1.5' }}>
                                {lastProgress ? (
                                  <>
                                    <div style={{ color: '#2563eb', fontWeight: 700, marginBottom: '6px', fontSize: '0.88rem' }}>[{lastProgress.date}]</div>
                    ...
                    <table style={{ width: '100%', tableLayout: 'auto', borderCollapse: 'collapse', marginBottom: '24px', fontSize: '0.88rem' }}>
                      <thead>
                        <tr style={{ background: '#f1f5f9', borderBottom: '2px solid #cbd5e1' }}>
                          <th style={{ padding: '6px 8px', border: '1px solid #cbd5e1', textAlign: 'center', verticalAlign: 'middle', width: '8%', lineHeight: '1.2' }}>구분</th>
                  ...
                  <div className="no-print" style={{ marginTop: '24px', borderTop: '1px solid var(--border)', paddingTop: '20px' }}>
                    <label style={{ fontSize: '0.9rem', fontWeight: 700, color: 'var(--text)', display: 'block', marginBottom: '8px' }}>📝 차주 주요 업무 및 요청사항 입력 (구글 시트 실시간 자동 저장/공유됨)</label>
                    <textarea
                      style={{ width: '100%', height: '120px', padding: '12px', border: '1px solid var(--border)', borderRadius: '8px', fontSize: '0.9rem', resize: 'vertical' }}
                      placeholder="다음 주 진행 업무 계획이나 팀장 결재/협조가 필요한 사항을 자유롭게 입력해 주세요. (입력 시 모든 사용자에게 구글 시트로 실시간 공유됩니다)"
                      value={nextWeekWork}
                      onChange={(e) => {
                        const val = e.target.value;
                        setNextWeekWork(val);
                        const weekKey = reportWeekRange.label.split(' (')[0];
                        localStorage.setItem(`weekly_report_next_week_work_${weekKey}`, val);
                        
                        // 구글 시트에 실시간 자동 저장 (디바운스 0.5초)
                        if (window._weeklyWorkTimer) clearTimeout(window._weeklyWorkTimer);
                        window._weeklyWorkTimer = setTimeout(() => {
                          fetch('/api/weekly-work', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ weekKey, content: val }),
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '24px', width: '100%', alignItems: 'start' }}>
              
              {/* 👈 [왼쪽 50%] Gmail / ECHO 아코디언 설정 카드 모음 */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>

                {/* 1️⃣ Gmail 설정 카드 (ON/OFF에 따라 컴팩트 1줄 접힘/펼침) */}
                <div className="panel" style={{ padding: '20px 24px', background: activeSmtpEngine === 'gmail' ? '#ffffff' : '#f8fafc', borderRadius: '14px', border: activeSmtpEngine === 'gmail' ? '2px solid #22c55e' : '1px solid var(--border)', transition: 'all 0.25s ease', boxShadow: activeSmtpEngine === 'gmail' ? '0 4px 20px rgba(34,197,94,0.12)' : 'none' }}>
                  
                  {/* 카드 헤더 (클릭 시 토글 접기/펼치기) */}
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }} onClick={() => setActiveSmtpEngine(activeSmtpEngine === 'gmail' ? 'none' : 'gmail')}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <h2 style={{ fontSize: '1.05rem', fontWeight: 800, color: 'var(--text)', margin: 0 }}>주간 자동 이메일 발송 설정 (Gmail 계정)</h2>
                          {activeSmtpEngine === 'gmail' ? (
                            <span style={{ background: '#dcfce7', color: '#15803d', fontSize: '0.72rem', fontWeight: 800, padding: '2px 8px', borderRadius: '99px' }}>실전 작동 중 (1순위)</span>
                          ) : (
                        ...
                        {activeSmtpEngine === 'gmail' && (
                          <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', margin: '3px 0 0 0' }}>
                            구글 지메일(Gmail) 서버를 활용해 사외 차단 없이 100% 자동 전송 및 요약을 수행합니다.
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  {/* ON 상태일 때만 세부 입력 폼 쫘악 펼침 (OFF일 때는 1줄 슬림 바만 깔끔하게 보임) */}
                  {activeSmtpEngine === 'gmail' && (
                    <div style={{ marginTop: '18px', paddingTop: '16px', borderTop: '1px dashed #cbd5e1' }}>
                      ...
                      <div style={{ marginBottom: '14px', background: '#f8fafc', padding: '12px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', alignItems: 'center', minHeight: '38px', padding: '6px 10px', background: '#ffffff', border: '1px solid var(--border)', borderRadius: '6px' }}>
                          ...
                          <input 
                            ...
                            style={{ border: 'none', outline: 'none', background: 'transparent', fontSize: '0.82rem', flex: 1, minWidth: '140px' }}
                          />
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '14px' }}>
                        <div>
                          ...
                          <input type="text" value={emailSettings.senderName || '마형석'} onChange={e => setEmailSettings({ ...emailSettings, senderName: e.target.value })} style={{ width: '100%', padding: '7px 10px', border: '1px solid var(--border)', borderRadius: '6px', fontSize: '0.82rem' }} />
                        </div>
                        <div>
                          <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, marginBottom: '4px' }}>📧 지메일 주소</label>
                          <input type="email" value={emailSettings.aiGmail || 'magma5018@gmail.com'} onChange={e => setEmailSettings({ ...emailSettings, aiGmail: e.target.value })} style={{ width: '100%', padding: '7px 10px', border: '1px solid var(--border)', borderRadius: '6px', fontSize: '0.82rem' }} />
                        </div>
                        <div>
                          <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, marginBottom: '4px' }}>🔑 앱 비밀번호 (16자리)</label>
                          <input type="password" value={emailSettings.gmailAppPassword || 'gojffulntemnfqfy'} onChange={e => setEmailSettings({ ...emailSettings, gmailAppPassword: e.target.value })} style={{ width: '100%', padding: '7px 10px', border: '1px solid var(--border)', borderRadius: '6px', fontSize: '0.82rem' }} />
                        </div>
                        <div>
                          <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, marginBottom: '4px' }}>👥 숨은참조 (BCC)</label>
                          <input type="email" value={emailSettings.bccEmail} onChange={e => setEmailSettings({ ...emailSettings, bccEmail: e.target.value })} placeholder="bcc@company.com" style={{ width: '100%', padding: '7px 10px', border: '1px solid var(--border)', borderRadius: '6px', fontSize: '0.82rem' }} />
                        </div>
                        <div>
                          <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, marginBottom: '4px' }}>⏱️ AI 답장 감지 주기</label>
                          <select value={emailSettings.checkInterval || '5'} onChange={e => setEmailSettings({ ...emailSettings, checkInterval: e.target.value })} style={{ width: '100%', padding: '7px 10px', border: '1px solid var(--border)', borderRadius: '6px', background: 'white', fontSize: '0.82rem' }}>
                        ...
                        <div>
                          ...
                          <div style={{ display: 'flex', gap: '4px' }}>
                            <select value={emailSettings.sendDay || 'MON'} onChange={e => setEmailSettings({ ...emailSettings, sendDay: e.target.value })} style={{ flex: 1, padding: '7px 8px', border: '1px solid var(--border)', borderRadius: '6px', background: 'white', fontSize: '0.88rem', fontWeight: 600 }}>
                            ...
                            <input 
                              type="time" 
                              value={emailSettings.sendTime || '09:00'} 
                              onChange={e => setEmailSettings({ ...emailSettings, sendTime: e.target.value })} 
                              style={{ flex: 1, padding: '6px 10px', border: '1px solid var(--border)', borderRadius: '6px', background: 'white', fontSize: '0.9rem', fontWeight: 700, fontFamily: 'inherit', color: '#0f172a', outline: 'none' }} 
                            />
                          </div>
                        </div>
                      </div>
                      ...
                      {/* 액션 버튼 모음 */}
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', justifyContent: 'flex-end', paddingTop: '10px', borderTop: '1px solid var(--border)' }}>
                        <button 
                          className="btn"
                          disabled={isTestingEmail}
                          style={{ background: '#22c55e', color: 'white', border: 'none', padding: '7px 12px', fontWeight: 700, fontSize: '0.78rem' }}
                          onClick={async () => {
                        ...
                        <button 
                          className="btn" 
                          disabled={isTestingEmail}
                          style={{ background: '#ef4444', color: 'white', border: 'none', padding: '7px 12px', fontWeight: 800, fontSize: '0.78rem' }}
                          onClick={async () => {
                            const validTargetRows = rows.filter(r => (r.autoEmail === 'Y' || r.autoEmail === 'y') && r.managerEmail && typeof r.managerEmail === 'string' && r.managerEmail.includes('@'));
                            if (validTargetRows.length === 0) {
                              alert('⚠️ 발송 가능한 대상이 없습니다! (자동발송 Y로 설정되고 담당자 이메일 주소가 작성된 사고만 발송됩니다)');
                              return;
                            }
                        ...
                        <button 
                          className="btn" 
                          style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', color: '#166534', padding: '7px 10px', fontWeight: 600, fontSize: '0.78rem' }}
                          onClick={async () => {
                            setIsTestingEmail(true);
                            const time = new Date().toLocaleTimeString('ko-KR');
                        ...
                        <button 
                          className="btn btn-primary" 
                          style={{ padding: '7px 14px', fontWeight: 700, fontSize: '0.78rem' }} 
                          onClick={async () => {
                          try {
                            const res = await fetch('/api/settings', {
                              method: 'POST',
                            ...
                            const data = await res.json();
                            if (data.success) {
                              alert('🎉 구글 시트 Settings 탭에 설정이 100% 영구 저장되었습니다!\n(어느 컴퓨터에서 접속하더라도 동일하게 완벽 유지됩니다)');
                            } else {
                ...
                {/* 2️⃣ ECHO 계정 설정 카드 (OFF 상태 시 1줄 컴팩트 헤더로 접어둠) */}
                <div className="panel" style={{ padding: '20px 24px', background: activeSmtpEngine === 'echo' ? '#ffffff' : '#f8fafc', borderRadius: '14px', border: activeSmtpEngine === 'echo' ? '2px solid #3b82f6' : '1px solid var(--border)', transition: 'all 0.25s ease', boxShadow: activeSmtpEngine === 'echo' ? '0 4px 20px rgba(59,130,246,0.12)' : 'none' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }} onClick={() => setActiveSmtpEngine(activeSmtpEngine === 'echo' ? 'none' : 'echo')}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <h2 style={{ fontSize: '1.05rem', fontWeight: 800, color: 'var(--text)', margin: 0 }}>주간 자동 이메일 발송 설정 (ECHO 계정)</h2>
                    ...
                    <div style={{ marginTop: '18px', paddingTop: '16px', borderTop: '1px dashed #cbd5e1' }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '14px' }}>
                        <div>
                          ...
                          <input type="text" placeholder="예: 마형석" style={{ width: '100%', padding: '7px 10px', border: '1px solid var(--border)', borderRadius: '6px', fontSize: '0.82rem' }} />
                        </div>
                        <div>
                          <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, marginBottom: '4px' }}>📧 전자 메일 주소</label>
                          <input type="email" placeholder="mhs810@hansol.com" style={{ width: '100%', padding: '7px 10px', border: '1px solid var(--border)', borderRadius: '6px', fontSize: '0.82rem' }} />
                        </div>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                        <button className="btn btn-primary" style={{ padding: '7px 14px', fontWeight: 700, fontSize: '0.78rem' }}>💾 ECHO 설정 저장</button>
                      </div>
                    </div>
                  )}
              ...
              {/* 👉 [오른쪽 50%] 실시간 테스트 발송 진행 로그 (상태 모니터링) 전면 배치 */}
              <div className="panel" style={{ padding: '20px 24px', background: '#0f172a', color: '#f8fafc', borderRadius: '14px', boxShadow: '0 10px 25px rgba(0,0,0,0.25)', border: '1px solid #1e293b', minHeight: '520px', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #334155', paddingBottom: '12px', marginBottom: '14px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <h3 style={{ margin: 0, fontSize: '0.98rem', fontWeight: 800, color: '#60a5fa' }}>실시간 테스트 발송 진행 로그 (상태 모니터링)</h3>
                    <div style={{ fontSize: '0.78rem', color: '#f59e0b', fontWeight: 700, background: '#1e293b', padding: '3px 10px', borderRadius: '6px', display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                      <span>⏳ [다음 AI 답장 자동 감지까지]:</span>
                      <strong style={{ color: '#38bdf8', fontSize: '0.85rem' }}>{countdownSec}초 남음</strong>
                      <span style={{ fontSize: '0.7rem', color: '#94a3b8' }}>({Math.floor(countdownSec / 60)}분 {countdownSec % 60}초)</span>
                    </div>
                  </div>
                  </div>
                  {testEmailLogs.length > 0 && (
                    <button 
                      ...
                      style={{ background: 'transparent', border: '1px solid #475569', color: '#94a3b8', borderRadius: '4px', padding: '2px 8px', fontSize: '0.72rem', cursor: 'pointer' }}
                    >
                      🗑️ 로그 지우기
                    </button>
                  )}
                ...
                {/* 로그 콘솔 창 */}
                <div style={{ flex: 1, background: '#020617', padding: '14px', borderRadius: '8px', border: '1px solid #1e293b', fontFamily: 'monospace', fontSize: '0.8rem', lineHeight: 1.6, overflowY: 'auto', maxHeight: '500px' }}>
                  {testEmailLogs.length === 0 ? (
                    <div style={{ color: '#475569', textAlign: 'center', marginTop: '130px' }}>
