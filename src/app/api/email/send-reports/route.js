...
export async function POST(req) {
  try {
    ...
    if (!fromEmail || !smtpHost) {
      return NextResponse.json({ success: false, error: '이메일 발송 설정을 먼저 확인해주세요.' }, { status: 400 });
    }

    const targetRows = rows.filter(r => (r.autoEmail !== 'N') && r.managerEmail && r.managerEmail.includes('@'));

    if (targetRows.length === 0) {
      return NextResponse.json({ success: false, error: '자동발송(Y)으로 지정되고 이메일이 입력된 사고 건이 없습니다.' }, { status: 400 });
    }
    ...
    for (const row of targetRows) {
      ...
      const aiReportText = row['AI 보고서 내용'] || row.aiReportText || 'AI 종합 분석 보고서가 생성되었습니다.';
      ...
      sentCount++;
      sentResults.push({ accNo, recipient: row.managerEmail });
    }
    ...
    return NextResponse.json({
      ...
      sentCount,
      sentResults,
      message: '총 ' + sentCount + '건의 사고 리포트가 담당자 이메일로 즉시 발송되었습니다.'
    });
